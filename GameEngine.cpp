//-----------------------------------------------------------------
// Game Engine Object
// C++ Source - GameEngine.cpp - version 2012 v2_10 HotFix3
...
#include "ListsController.h" // <- MasterOfLists

//-----------------------------------------------------------------
// Static Variable Initialization
//-----------------------------------------------------------------
GameEngine* GameEngine::m_GameEnginePtr = 0; // signleton class!! ==evil   >:D
...
DWORD WINAPI MyThreadProc(GameEngine* gamePtr)
{
	return gamePtr->MyThreadProc();
}
							...
							m_UserFontPtr(0)
{
	...
	m_hKeybThread = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE) ::MyThreadProc, this, NULL, &m_dKeybThreadID);
}
...
DWORD GameEngine::MyThreadProc()
{
	return 0;

	while( MasterOfLists::GetSignleton() == 0 ){
		Sleep(2); // wacht totdat de Sginleton is gemaakt...
	}

	StopWatch *localTickTimerPtr = new StopWatch();
	localTickTimerPtr->Reset();
	localTickTimerPtr->Start();

	while( m_bKeybRunning ){


		//Tick timing is independent from Paint timing
		double tickTimeTrigger = 0.008;
		localTickTimerPtr->Tick();
		double tickTimeNow = localTickTimerPtr->GetGameTime();
		if(tickTimeNow > tickTimeTrigger)
		{
			localTickTimerPtr->Reset();
			localTickTimerPtr->Start();
		}
		if( tickTimeNow >= 0.2 ) DebugBreak();
		...
		for (unsigned int plaats=0; plaats<dynamicPtrVect.size(); ++plaats){
			if( dynamicPtrVect.at(plaats) != 0){
				dynamicPtrVect.at(plaats)->RayBounceWrapper(tickTimeNow);
			}
		}

		int sleepTime= (int)(1000*tickTimeNow);
		Sleep(sleepTime);
	}
	Sleep(1);
	return 0;
...
void GameEngine::CreateGeometry(ID2D1PathGeometry& pathRef, const POINT ptsArr[], int count, bool close)
{ /*
	ID2D1GeometrySink *pSink = NULL;
	ID2D1Factory *m_pD2DFactory;
	ID2D1PathGeometry *m_pPathGeometry;
	m_pPathGeometry= &pathRef;

	// Create a path geometry.
	HRESULT hr;
	if (SUCCEEDED(hr))
	{
		hr = m_pD2DFactory->CreatePathGeometry(&pathRef);
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (close) pSink->AddLine(D2D1::Point2F(0, 0));
...
{	
	MessageBox("werkt nog niet");
	/*ID2D1PathGeometry path;
	CreateGeometry(path, ptsArr, count, false);

	m_RenderTargetPtr->FillGeometry(path);//*/
	return true;
}/*
bool GameEngine::FillPolygon(const POINT ptsArr[], int count, bool close)
{
	ID2D1PathGeometry path;
	CreateGeometry(path, ptsArr, count, close);
...
{
	//if (ratio > 0.2 && ratio < 10) m_TickPaintRatio = ratio; // niet handig!
	m_TickPaintRatio= min(10, max(0.2, ratio ) );
	if (ratio < 0.2 && ratio > 10)
		OutputDebugString( "\nTickPaintRatio is too Large! or to smal.");
}
