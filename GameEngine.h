...
class Midi;
class String;
//class GraphicsEngine; // word niet gebruikt
//class Painter;
class StopWatch;
//class Bitmap;
class TextFormat;
...
struct DOUBLE2
{
	...
	// Methods
	// -------------------------
	double			DoubleLength(){return x*x +y*y;}
	double			DoubleLength(DOUBLE2 other){return (x-other.x)*(x-other.x) +(y-other.y)*(y-other.y);}
...
public:
	...
	//FillPolygon is a lie!
	bool			FillPolygon(const POINT ptsArr[], int count);
	bool			FillPolygon(const POINT ptsArr[], int count, bool close);
	void			CreateGeometry(ID2D1PathGeometry& pathRef, const POINT ptsArr[], int count, bool close);
	...
	double  GetTickPaintRatio(){ return m_TickPaintRatio; } // zelf toegevoegd
	...
	virtual DWORD MyThreadProc(); // myThread!
